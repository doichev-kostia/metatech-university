...
const { createHttpServer, getHost } = require("./http-utils.js");
...
module.exports = (root, port, { secure = false } = {}) => {
    ...
    const server = createHttpServer(handler, { secure }).listen(port);

    const addr = server.address();
    let protocol = secure ? "https" : "http";
    let host = getHost(addr);

    logger.log(`Static is listening on ${protocol}://${host}🚀`);
};
